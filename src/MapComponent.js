...
const RenderMap = ({ val, index, showPopup }) => {
  const fColor = val?.Type ? colors[val.Type.slice(-1)] : "red";
  return (
    <div key={index}>
      <Marker latitude={val?.Lat} longitude={val?.Lon}>
        <Pin color={fColor}></Pin>
      </Marker>
      {showPopup && (
        <Popup
          longitude={val?.long || -77}
          latitude={val?.latd || 38}
          anchor="bottom"
        >
          <div>{val?.Type}</div>
          <div>{val?.Timestamp_str}</div>
        </Popup>
      )}
    </div>
  );
...
const MapComponent = () => {
  ...
  const handleNavChange = (e) => {
    console.log("handleNavChange", e);
  };

  useEffect(() => {
    callApiData();
  }, []);
  if (data) console.log(data.Points);
  // const { Name, NoOfPoints, Points } = data;
  return (
    <div
    ...
    >
      ...
      >
        ...
        {newPlace ? (
          <Marker
            latitude={newPlace?.latd || 38}
            longitude={newPlace?.long || -77}
          >
            <Pin color={newPlace?.color}></Pin>
          </Marker>
        ) : null}
        {showPopup && (
          <Popup
            longitude={(newPlace && newPlace?.long) || -77}
            latitude={(newPlace && newPlace?.latd) || 38}
            anchor="bottom"
            onClose={() => setShowPopup(false)}
          >
            <div>{data && data.Points && data.Points[0].Type}</div>
            <div>{data && data.Points && data.Points[0].Timestamp_str}</div>
          </Popup>
        )}
        <NavigationControl />
        <ScaleControl />
